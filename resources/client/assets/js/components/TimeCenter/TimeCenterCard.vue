<template>
  <div class="project-cell">
    <router-link
      :to="{ name: 'timecenter.show', params: { id: project.id } }"
      class="icon"
    >
...
export default {
  ...
  computed: {
    ...
    creatorImage0() {
      return this.project.users[0] ? this.project.users[0] && '/storage/' + this.project.users[0].avatar : this.defaultImage ;
    },
    creatorImage1() {
      return this.project.users[1] ? this.project.users[1] && '/storage/' + this.project.users[1].avatar : this.defaultImage ;
    },
  },
  created() {
    // Code...

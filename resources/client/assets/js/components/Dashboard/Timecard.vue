  ...
  <section>
    <a href="#" @click="handleTimecard">
      <div class="timecard">
        <div class="userinfo">
          <img
            :src="'/storage/' + data.userData.avatar"
    ...
    >
      <div slot="header" style="width: 100%">
        <div class="row">
          ...
          <div class="col-sm-8" style="margin-top: auto; margin-bottom: auto; text-align: center; display: flex;">
          ...
          <table class="timecard-period-table">
            ...
            <tbody>
              <tr
                ...
                v-bind:key="item.id"
                v-bind:class="{ today: item.today }"
              >
                <td width="120">{{ item.day }}</td>
                <td width="60">
                  <div
                    class="checkin align-items-center justify-content-center"
                  >
                    <div
                      @click="showCheckinModal(item.day, item.date)"
                      class="plus-icon"
                      v-show="item.length === 0"
                    >
                      +
                    </div>
                    <div class="checkin-value" v-show="item.length > 0">
                      {{ item.length }}
                    </div>
                    <a
                      href="#"
                ...
                <td width="60">
                  <div
                    class="checkout align-items-center justify-content-center"
                  >
                    <div
                      @click="showCheckinModal(item.day, item.date)"
                      class="plus-icon"
                    ...
                    <div class="checkout-value" v-show="item.length > 0">
                      {{ item.length }}
                    </div>
                    <a
                      href="#"
                ...
                <td>
                  <div class="entry align-items-center">
                    <div @click="showEntryModal(item.date)" class="plus-icon">
                      +
                    </div>
                    <div
                      class="duration"
                      v-show="entry.entry_time_type"
                      v-for="entry in item"
                      v-bind:key="entry.id"
                      @click="editEntry(entry, item.date)"
                    >
                      {{ entry.format_duration }}
                      {{ entry.entry_time_type }}
                    </div>
                  </div>
                </td>
                <td width="60">
      ...
      <div slot="body">
          ...
          <div v-show="checkin.check_in || checkin.check_out">
            <div class="botbar d-flex justify-content-between">
              <div class="align-items-center" v-show="checkin.check_in">
                ...
                <div class="checked">
                  <div class="checkedin text-success font-weight-bold">
                    Checked in
                  </div>
              ...
              <div class="align-items-center" v-show="!checkin.check_in">
                <a
                  href="#"
                  class="checkedout text-primary font-weight-bold"
                  @click="showTimeModal('checkin', checkin.id)"
                  ><span class="fa fa-plus"></span>Add Checkin</a
                >
              </div>
              <div class="align-items-center" v-show="checkin.check_out">
                ...
                <div class="checked">
                  <div class="checkedout text-primary font-weight-bold">
                    Checked out
                  </div>
              ...
              <div class="align-items-center" v-show="!checkin.check_out">
                <a
                  href="#"
                  class="checkedout text-primary font-weight-bold"
                  @click="showTimeModal('check_out', checkin.id)"
                  ><span class="fa fa-plus"></span>Add Checkout</a
                >
              </div>
              <div
                class="align-items-center"
                v-show="checkin.check_in || checkin.check_out"
              >
                <a href="#" @click="removeCheckin(checkin.id)"
        ...
        <div
          class="
            ...
            mb-3
          "
          @click="showTimeModal('check_in', 0)"
        >
          <span class="fa fa-plus"></span> <strong>Add Checkin</strong>
        </div>
        <div
          class="d-flex justify-content-center align-items-center checkin-time"
          @click="addComepleteCheckin"
        >
          ...
          <div class="complete-checkin" v-show="isCompleteCheckin">
            <div class="d-flex">
              ...
              <div class="form-inline">
                <select
                  class="form-control pt-2 px-3 mr-2"
                ...
                <select class="form-control pt-2 px-3" v-model="checkinMinutes">
                  <option
                    :value="minute"
                    v-for="(minute, i) in 60"
                    v-bind:key="minute.id"
                  >
                    {{ i }}
                  </option>
                </select>
    ...
    >
      ...
      <div slot="footer" class="w-100">
        <div class="w-100 d-flex justify-content-end">
          ...
          <button class="btn btn-success" type="button" @click="addCheckin">
            Choose
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getWeekLabel(offsetWeek){
      const weekStart = moment().day("Monday").week(moment().week() + this.offsetWeek).format("MM.DD");
      const weekEnd = moment().day("Sunday").week(moment().week() + this.offsetWeek).format("MM.DD/YYYY");
...
.form-group .form-control:not(.ui-select-multiple),

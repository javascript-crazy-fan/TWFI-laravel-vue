...
const getters = {
	...
	offsetWeek: (state) => state.offsetWeek,
...
const actions = {
	...
	//     })
	// },
	prevWeek({ commit, state, dispatch }, param) {
		commit('setOffsetWeek', state.offsetWeek - 1);
		dispatch('fetchWeekData', {
			userId: param.userId,
			projectId: param.projectId,
			startDate: moment()
				.day("Monday")
				.week(moment().week() + state.offsetWeek)
				.format("YYYY-MM-DD"),
			endDate: moment()
				.day("Sunday")
				.week(moment().week() + state.offsetWeek)
	...
	curWeek({ commit, state, dispatch }, param) {
		commit('setOffsetWeek', 0);
		dispatch('fetchWeekData', {
			userId: param.userId,
			projectId: param.projectId,
			startDate: moment()
	...
	nextWeek({ commit, state, dispatch }, param) {
		commit('setOffsetWeek', state.offsetWeek + 1);
		dispatch('fetchWeekData', {
			userId: param.userId,
			projectId: param.projectId,
	...
	newAddEntry({ commit, dispatch }, param) {
		axios
			.post('/api/v1/timecards/new-add-entry', {
				isUpdate: param.isUpdate,
				isCompleteCheckin: param.isCompleteCheckin,
				id: param.id,
				userId: param.userId,
...
const mutations = {
	...
	setOffsetWeek(state, offsetWeek) {
		state.offsetWeek = offsetWeek
	},
	setData(state, items) {
		items.forEach(timecard => {
			...
			if (timecard.staticData != null) {
				...
				if (Math.floor(todayTime % 60) < 10) {
					timecard.staticDuration = (todayTime > 0) ? Math.floor(todayTime / 60) + ':' + '0' + Math.floor(todayTime % 60) : '0:00'
				}
				else {
					timecard.staticDuration = (todayTime > 0) ? Math.floor(todayTime / 60) + ':' + Math.floor(todayTime % 60) : '0:00'
				}
				if (Math.floor(weekTime % 60) < 10) {
					timecard.weekTime = (weekTime > 0) ? Math.floor(weekTime / 60) + ':' + '0' + Math.floor(weekTime % 60) : '0:00'
				}
				else {
	...
	setWeekData(state, items) {
		...
		if (Math.floor(weekTime % 60) < 10) {
			state.weekData.weekTime = (weekTime > 0) ? Math.floor(weekTime / 60) + ':' + '0' + Math.floor(weekTime % 60) : '0:00'
		}
		else {

...
class UsersController extends Controller
{
  ...
  private function _updateCertificates($userid, $certificate_name, $certificate_description){
  ...
  public function store(StoreUsersRequest $request)
  {
    ...
    $this->_updateCertificates($user->id, $request->certificate_name, $request->certificate_description);

    $user_email = $this->_getUserInfo($user->id)->toArray();
    $user = $this->_getUserInfo($user->id);
    // Email sending when create a new user.
    if ($request->send_invitation_email==1){
      Mail::send('mail', $user_email, function($message) use ($user_email){
        $message->to($user_email['email'])->subject("New Account");
        $message->from("ttroutman@twfi.net", "");
      });
      echo "Email Sent!!!";
    }      
